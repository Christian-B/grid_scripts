   ...
   try:
      ...
      with open_sam(args[0], opts.samtype) as (pe_mode, read_seq):
          with get_write_to_samout(opts.samout, pe_mode) as write_to_samout:  
             if opts.stranded in ["yes", "no", "both"]:
             ...
             if opts.stranded in ["reverse", "both"]:
             ...
             if pe_mode:
                 count_reads_paired(read_seq, forward_counter, reverse_counter, opts.order, 
                             opts.quiet, opts.minaqual, write_to_samout )
             else:                
                 count_reads_single(read_seq, forward_counter, reverse_counter, opts.order, 
                             opts.quiet, opts.minaqual, write_to_samout )
             if forward_counter is not None:
                 print "forward"
                 forward_counter.small_results()
                 #forward_counter.results()
             if reverse_counter is not None:
                 print "reverse"
                 reverse_counter.small_results()
                 #reverse_counter.results()
   except:
      sys.stderr.write( "  %s\n" % str( sys.exc_info()[1] ) )
