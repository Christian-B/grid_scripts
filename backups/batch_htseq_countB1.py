...
class Both_IV_Counter():
    ...
    def results(self, stream):
        self.forward.results(stream)
        self.reverse.results(stream)
...
def iv_counter_factory(features, counts, write_to_samout, mode, stranded):
    if stranded in ["yes", "no", "both"]:
        if mode == "union":
            forward = Forward_Union_IV_Counter(features, counts, write_to_samout)
        elif mode == "intersection-strict":
            forward = Forward_Strict_IV_Counter_Strict(features, counts, write_to_samout)
        elif mode == "intersection-nonempty":
            forward =  Forward_Nonempty_IV_Counter_Nonempty(features, counts, write_to_samout)
        else:  
            sys.exit("Illegal overlap mode: " + mode)
    if stranded in ["reverse", "both"]:
        if mode == "union":
            reverse = Reverse_Union_IV_Counter(features, counts, write_to_samout)
        elif mode == "intersection-strict":
            reverse = Reverse_Strict_IV_Counter_Strict(features, counts, write_to_samout)
        elif mode == "intersection-nonempty":
            reverse =  Reverse_Nonempty_IV_Counter_Nonempty(features, counts, write_to_samout)
        else:  
            sys.exit("Illegal overlap mode: " + mode)
    ...
    if stranded == "reverse":
        return reverse
    if stranded == "both":
        return Both_IV_Counter(forward, reverse)    
...
def count_reads_using_features(sam_filename, features, counts, samtype, order, stranded, 
    ...
    with open_sam(sam_filename, samtype) as (pe_mode, read_seq):
        with get_write_to_samout(samout, pe_mode) as write_to_samout:  
            counter = iv_counter_factory(features,
                                                     counts,
                                                     write_to_samout,
                                                     overlap_mode, stranded)
            if pe_mode:
                pass
                #count_reads_paired(read_seq, forward_counter, reverse_counter, order, 
                #                   quiet, minaqual, write_to_samout )
            else:                
                count_reads_single(read_seq, counter, order, 
                                   quiet, minaqual)
