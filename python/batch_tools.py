        ...
        if parent:
            if not os.path.isdir(parent):
                if os.path.isfile(parent):
                    raise Exception("parent: " + parent + " is a file.")
                grand_parent = os.path.dirname(os.path.normpath(parent))
                if os.path.isdir(grand_parent):
                    os.mkdir(parent, 0744)
                else:
                    raise Exception("grand parent of " + parent + " does not exist.")
            if not os.path.isdir(parent):
                raise Exception("parent: " + parent + " does not exist.")
...
class ByPrefixCombiner(File_action):
    ...
    def __init__(self, divider, use_files, parent=os.getcwd(), output_dir=None, strip=True):
...
def command_error(parser, error):
    print error
    parser.print_help()
    print
    print error
...
if __name__ == '__main__':
    ...
    if len(args) == 0:
        command_error(parser, "No command specified! Legal commands are: " + str(__COMMANDS__))

    for arg in args:
        if arg not in __COMMANDS__:
            command_error(parser, "Unexpected command " + arg + " Legal commands are: " + str(__COMMANDS__))

    endings_mappings = None
    required_files = None
    if __FIND__ in args:
        if __MERGE__ in args:
            command_error(parser, __FIND__ + " and " + __MEGRE__ + " can not be combined due to different " + FILE_LIST + " formats")
        if not options.__dict__[__FILE_LIST__]:
            command_error(parser, __FIND__ + " selected but no " + __FILE_LIST__ + " parameter provided")
        endings_mappings = {}
        for file_option in options.__dict__[__FILE_LIST__]:
            parts = file_option.split(":")
            if len(parts) != 2:
                print parts
                command_error(parser, __FILE_LIST__ + " " + file_option + " not in the expected regex:name format")
            endings_mappings[parts[0]] = parts[1]
        required_files = endings_mappings.values()
    elif __MERGE__ in args:
        if not options.__dict__[__FILE_LIST__]:
            command_error(parser, __MERGE__ + " selected but no " + __FILE_LIST__ + " parameter provided")
        file_mappings = {}
        for file_option in options.__dict__[__FILE_LIST__]:
            parts = file_option.split(":")
            if len(parts) != 2:
                command_error(parser, __FILE_LIST__ + " " + file_option + " not in the expected ending:name format")
        ...
        for file_option in options.__dict__[__FILE_LIST__]:
            ...
            if len(parts) == 1:
                ...
                command_error(parser, "FILE_LIST " + file_option + " has more than one : in it.")
    ...
    if __LIST__ in args:
        ...
        if not required_files:
            command_error(parser, __LIST__ + " selected but no " + __FILE_LIST__ + " parameter provided")
    ...
    if __BATCH__ in args:
        if not options.__dict__[__QSUB_SCRIPT__]:
            command_error(parser, __BATCH__ + " selected but no " + __QSUB_SCRIPT__ + " parameter provided")
    ...
    if __EXTRACT__ in args:
        if not options.__dict__[__EXTRACT_PREFIX__]:
            command_error(parser, __EXTRACT__ + " selected but no " + __EXTRACT_PREFIX__ + " parameter provided")
        if not required_files:
            command_error(parser, __EXTRACT__ + " selected  but no " + __FILE_LIST__ + " parameter provided")
    ...
    if __DELIMIT__ in args:
        if not options.__dict__[__DELIMITER__]:
            command_error(parser, __DELIMIT__ +
                          " selected but no " + __DELIMITER__ + " parameter provided")
        if not required_files:
            command_error(parser, __DELIMIT__ +
                          " selected  but no FILE_LIST parameter provided")
